...
func CruciblePlcDataOperation(interval time.Duration) {
	...
	for _, device := range devices { //设备遍历循环
		...
		//根据filter条件查询历史表
		curr, err := global.CrucibleHisDataColl.Find(context.TODO(), filter, &opts)
		if err != nil {
			log.Println("坩埚PLC设备历史数据", err)
			return
		}
		...
		for i := range boxDB {
			...
			if i == 0 { //数据格式初始化，数据初始化
				...
				for k := range boxDB[i].Data[0].Detail {
				...
				for k := 0; k < dataLen; k++ {
					...
					if boxDB[i].Data[0].Detail[k].Value == "0" || boxDB[i].Data[0].Detail[k].Value == "1" {
						...
						//fmt.Println(info.Data[0].Detail[k].Value,info.Data[0].Detail[k].Key, "11111", boxDB[i].Data[0].Detail[k].Key)
		...
		cruciblePlcColl(info, interval)
	}

}

func cruciblePlcColl(box model.Box, interval time.Duration) {
	var coll *mongo.Collection
	var msg string
	switch interval {
	case 10 * time.Minute:
		coll = global.CrucibleTenMinDataColl
		msg = "10分钟"
	case 30 * time.Minute:
		coll = global.CrucibleThirtyDataColl
		msg = "30分钟"
	case time.Hour:
		coll = global.CrucibleHourDataColl
		msg = "小时"
	default:
		fmt.Println("坩埚PLC时间粒度选择错误")
		return
	}
	_, err := coll.InsertOne(context.TODO(), box)
	if err != nil {
		fmt.Println("坩埚PLC" + msg + "存储失败")
		return
	}
	fmt.Println("坩埚PLC" + msg + "存储成功")
	return
}
