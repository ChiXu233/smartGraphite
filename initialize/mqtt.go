...
// 使用Mqtt接收瑞通新服务器的CEMS消息
func MqttInitNew(id string, mqttClient *mqtt.Client) {
	//配置
	address := "tcp://119.45.136.206:1883"
	//设置地址用户名和密码
	clientOptions := mqtt.NewClientOptions().AddBroker(address).SetUsername("admin101").SetPassword("sdl@admin101")
	//设置客户端id
